...
(function () {
  ...
  ReMix._resolveVariable = function (match, key) {
    ...
    if (!ReMix.registered[key])
      return match;
    return '(?:' + ReMix.registered[key].source + ')' + mod;
  };
  /**
  ...
  ReMix.register = function (identifier, spec) {
    ...
    if (ReMix.registered[identifier])
   ...
   * @public
   * @static
   * @param {String} identifier - Registered identifier to be removed.
   * @returns {ReMix}
   * @category Template
   */
  ReMix.unregister = function (identifier) {
    ReMix.registered[identifier] = null;
    return ReMix;
  };
  /**
   ...
   * @returns {ReMix}
   */
  ReMix.prototype.clone = function () {
    var Klass = this.constructor;
    return new Klass(this.name(), this._specs);
  };
  /**
